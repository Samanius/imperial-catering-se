...
export default function TastingMenu({ restaurantName, description, menuItems }: TastingMenuProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="linen-texture bg-card border border-accent/20 shadow-lg rounded-sm p-12 md:p-16">
        {description && (
          <div className="text-center mb-12 pb-8 border-b border-accent/30">
            <p className="font-body text-base leading-relaxed text-foreground/80 italic">
              {description}
            </p>
          </div>
        )}
        ...
        {Object.entries(categorizedItems).map(([category, items], index) => (
          <div key={category} className="mb-10">
            <div className="text-center mb-6">
              <h3 className="font-heading text-2xl font-semibold tracking-wider uppercase relative inline-block">
              ...
              {items.map((item) => (
                <div key={item.id} className="flex justify-between items-start gap-4">
                  <div className="flex-1">
                    <h4 className="font-heading text-lg font-medium mb-1">
                      {item.name}
                    </h4>
                    {item.description && (
                  ...
                  <span className="font-body text-base text-muted-foreground whitespace-nowrap ml-4">
                    {item.price}
                  </span>
                </div>
              ))}
        ...
        <div className="mt-12 pt-8 border-t border-accent/30 text-center">
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40">
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button
              size="lg"
              className="bg-accent text-accent-foreground hover:bg-accent/90 font-body tracking-wider shadow-xl"
            >
              Concierge Order
            </Button>
          </DialogTrigger>
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="order-message" className="font-body">
              ...
              <Button
                ...
                className="w-full bg-accent text-accent-foreground hover:bg-accent/90 font-body tracking-wider"
              >
